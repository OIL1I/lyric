...
Current version: **1.3.1**, bytecode format **3.1**.
...
| `lyrls` | Language server over stdio, started by an editor: diagnostics, hover, go to definition, outline, find references |

`lyrembed.dll` is the host library: compile and run Lyric from C#.

```
$ lyric repl
Lyric 1.3.1 — :help for commands, :quit to leave
lyr> let x = 5
lyr> x * 2
10
```
